...
3. **Look up facts; ask decisions.** If a fact is discoverable from the codebase, the
   specs (`specifications/`), go read it — never ask the user for it.
   Only *decisions* go to the user.
4. **Do not act until the user confirms shared understanding.** The output of a grilling
   is an agreed position — record it as a decision record in `specifications/decisions/`
   before implementation starts.
5. Check what is already decided first: `specifications/decisions/README.md` — do not re-grill
   settled decisions unless the user brings new evidence.
