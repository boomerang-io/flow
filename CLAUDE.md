...
| DD-03 | **Unified product versioning** — one tag builds the compatible image set; no independent engine version line. SHIPPED: `ci-release.yml` builds all four images off one `5.x.y` tag; `:latest` moves on stable tags only. The `engine@` alias tags were **ruled OUT (2026-09-01)** — v5 has never shipped, so no v5 embedder exists to deprecate; v4 embedders stay on the v4 image tags. Reopen only if a v5 embedder appears needing an engine-only pull line. |
| DD-04 | **Frontend (`flow.client.web`) joins this monorepo** — after the merged image ships; continues from the **v4 line** (`main`, at tag `4.0.0`+3; the package.json version field is cosmetic, pinned `5.0.0` — releases are tag-driven, as on the backend), joining the DD-03 unified product version at 5.x; webapp served only in `standalone` mode (AM-7).           |
...
- **Atomic claiming via `findAndModify`** — no distributed locks. Claims carry `claim.by`/
  ...
  add the semantics. **Leases are real since 2026-09-01** (`feat-v5-liveness`, closing AM-3's
  "no consumer" deferral): the dispatcher's executor threads stamp a local `LeaseRegistry`, one
  `LeaseHeartbeat` per dispatcher batches `PUT /api/v1/dispatcher/{id}/heartbeat {ids}` every
  `flow.dispatcher.lease.beat-ms=30000`, the engine `renewLeases` (one `updateMulti`, fenced on
  `claim.by`) into `claim.leaseExpiresAt` with `flow.dispatcher.lease-ms=90000`, and the
  `reapExpiredLeases` sweep requeues/abandons with `statusReason=LeaseExpired`. Opt-in by
  behaviour: a dispatcher that never heartbeats never holds a lease and falls back to
  `reapClaimsFromGoneDispatchers` (60 s staleness) and `timeoutAt`. Crash recovery is therefore
  lease-then-deadline; the lease MUST stay ≥ 2× the beat + one sweep interval (90 s is the floor).
  The dispatcher side also replaced "wait once on the Kubernetes watch" with a reconcile loop
  (`kube.timeout.reconcileSeconds=30`, watch re-open on loss, adopt-existing-Job; `TaskWatcher`
  no longer `System.exit(1)`s) and reports a typed `statusReason` beside `statusMessage`
  (closed set on `TaskRunEndRequest`; M4's ruled `failureClass` is a derivation of it, not a
  second wire field). Design record: `specifications/competitive-analysis.md` §15, §18.
...
| `specifications/competitive-analysis.md` | 📎 Research record (2026-09-01) | Eight products vs the engine/dispatcher: IBM Cloud Code Engine, GitHub Agentic Workflows (`gh-aw`), Temporal, Trigger.dev, Vercel Workflows, Kestra, n8n, Langflow. Per-product adopt/conflict tables with `file:line`, a cross-product synthesis (§15 — 11 converged themes; liveness is the one place Flow is the outlier, recorded for the AM-3 review), and one ranked action list (§16). Design inputs for the propose/dispose node in `repo-insights-engagement-inputs.md` §5. |
...
rebuild (claims/CAS + `claim.seq` fencing, eleven `WorkflowWatcher` sweeps, pause as a single admission
