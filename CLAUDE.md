...
| ----------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------ |
| `specifications/v5-enhancemnet.md`        | 🔴 **MASTER — start here**   | The authoritative v5 spec: phases, Q-register (answers recorded in place), Living Sections, §10 Decisions (DD-01…DD-08) + the pre-v5 ADR/AD table (ADR001–003, AD001–004: retained/superseded/moot) and the DD-08 labels-vs-annotations convention; Q-007 proposes removing GSON. Note the filename typo is historical — keep it. |
...
| `specifications/queue-design.md`          | ✅ Ruled (2026-07-23)        | Q-225: claim/queue design (4 classes), WorkflowWatcher spec (6 sweeps), scheduling-substrate comparison (decision deferred) + the ADR001 (2022 async/sync) prior-decision note resolved as poll-and-claim. |
| `specifications/multi-instance-model.md`  | ✅ Ruled (2026-07-23)        | Q-222/223/224: write discipline (CAS-only), eventing (outbox/no broker/no partitioning), reconciler convergence + the outcome line closing the pre-v5 NATS/dapr roadmap. |
| `specifications/gap-register.md`          | ✅ Delivered                 | Q-220/226: lock inventory, master gap list (BEFORE/WITH/POST-MERGE), Phase 3 backlog E0–E11 + the 12 Phase 0 test scenarios; §G carries the relationship-layer design lineage (Models 1–4, `rel_nodes`/`rel_edges` schema, JGraphT rejected in E6). |
...
| `specifications/scaling.md`               | 📎 Annex                     | Phase 2B locking/queueing brief + the 2021 fio storage-class benchmarks appendix (GlusterFS / Ceph SSD / IBM VPC block). |
| `specifications/design-system.md`         | 📎 Reference                 | IBM Carbon + Boomerang theme design system (source of truth: `flow.client.web`) + browser support (AD004 vs the actual `browserslist`). |
...
| `specifications/entity-diff-v4-v5.md`      | ✅ Reviewed + actioned (2026-08-21); Part A added 2026-09-01 | Now v3→v5: **Part A** is the v3→v4 field-by-field collection map reconciled against loader units `_0004`…`_0014` (drift list A.4) plus the ADR003 versioning pattern (subset pattern, implemented); **Part B** is the field-level v4→v5 diff. **Part B §7 documents the accepted outbox creation-loss window** — no open G2 items remain. |
| `specifications/api-contract-trace.md`     | 🔵 **ACTIVE (2026-08-18)**   | End-to-end webapp↔service-core contract trace (call site → route → service → serialised fields). §0 feature intent for the workflows / activity / insights clusters, the webhook-event cluster (CloudEvent sample; the old invalid-token rule no longer holds), live defects, blocked capability, the permissions/auth findings that gate the frontend work, and the maintainer decisions outstanding. |
...
| `specifications/community-issue-triage.md` | 📋 Audit trail (2026-09-01)  | The `boomerang-io/community` issue-tracker sunset: all 143 open issues triaged one by one (61 transferred to `flow`/`bosun.*`/`tasks`/`website`, 82 closed as completed/duplicate/not planned) with the destination or reason per row. |
