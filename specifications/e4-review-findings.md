...
| D11 | `AgentService` long-poll | N per-candidate `findAndModify` claims per poll + blocking `Thread.sleep(1000)` on a platform thread for the 30s hold. One parked platform thread per connected agent → ~200-agent ceiling at the default Tomcat pool. | ✅ **MEASURED + DECIDED (Track 3 / E7 opener, 2026-08-13).** `AgentPollerVirtualThreadTest` drove 200 concurrent virtual-thread pollers through the real `findClaimable` Mongo path + idle wait under a JFR `jdk.VirtualThreadPinned` recording: **0–2 pins across 40k operations** (one-time class-init / pool-warmup, NOT per-operation) — the claim path does not pin on Java 25 (JEP 491) + driver 5.8.0. **Decision (b): `spring.threads.virtual.enabled=true` on the engine.** Parked pollers now cost a cheap virtual thread, removing the platform-thread ceiling; the full engine suite runs green under VT. Test kept as a pinning **regression guard** (asserts pins stay <1% of poll cycles). **Residual NOT fixed by (b)** — the `Thread.sleep(1s)` busy-poll still issues ~1 idle DB query/sec/agent, so idle DB load scales with agent count. That is a **poll-shape** problem (option (c): async/`DeferredResult` or event-driven poll), carried into the E7 protocol-v2 design rather than built standalone (protocol v2 may make the poll lease-based/event-driven and subsume it). |
