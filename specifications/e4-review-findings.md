...
| D11 | `AgentService` long-poll | N per-candidate `findAndModify` claims per poll + blocking `Thread.sleep(1000)` on a platform thread for the 30s hold (no virtual threads set; no async). One parked platform thread per connected agent. | **DEFERRED to E7 (Track 3) as a measurement gate, NOT a blind code change.** Rationale: correctness is fine — this is thread-efficiency under scale. (1) The one-line fix (`spring.threads.virtual.enabled=true`) can't be trusted without measuring **pinning** — a VT that blocks in a `synchronized`/native section (Mongo driver checkout, claim path) pins its carrier and changes nothing; verify with JFR `jdk.VirtualThreadPinned`. (2) May not be needed at realistic agent counts (~200 platform threads is plenty for tens of agents). (3) The `Thread.sleep` busy-wait hints the better answer may be an **async/`DeferredResult`** long-poll that holds no thread while idle, not just cheaper threads. **Collides with E7**: the whole poll protocol is reworked there (protocol v2, worker leases), which may replace this long-poll wholesale — so run the load harness (K agents, seeded DB → platform-thread occupancy, throughput, p99 claim latency, pinning events) as the **opening move of E7**, before touching the dispatcher protocol, and decide among: (a) leave on platform threads, (b) flip virtual threads if pollers unmount cleanly, (c) rewrite the poll async. This is the "new claim poller" Q-005 (Phase 0) said to measure. |
