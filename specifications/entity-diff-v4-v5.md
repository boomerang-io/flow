...
| 2 | `claim.leaseExpiresAt` declared + `lease_sweep` indexed, never written | ~~**Left** — AM-3 (leases deferred)~~ **WRITTEN since 2026-09-01** — `TaskRunService.renewLeases` on the dispatcher heartbeat; read by `findLeaseExpired` in the `reapExpiredLeases` sweep. The index is live. `statusReason` (String, no index) was added to `TaskRunEntity`/`TaskRun`/`TaskRunEndRequest` in the same slice as the typed cause beside `statusMessage`. |
