...
| A3 | Agent endpoints unauthenticated | C | ✅ **INTERIM SHIPPED (E7-4)**: static shared-secret bearer token on `/api/v1/dispatcher/**` (`DispatcherAuthFilter`, enforce-when-configured). **DEFERRED to POST-MERGE (DD-02)**: replace with a first-class Flow token — new `AuthScope.dispatcher` + `bfd` prefix, minted via the normal token API, engine validates by SHA-256 hash against the shared `tokens` collection. Held until the merge so the engine has the in-process token system rather than reaching into flow-owned data pre-merge. **Before implementing, check ARCHIE (`~/Workspaces/tlawrie/asdr`) — it has the most recent token impl (changed FE→BE token flow + revalidation): `specifications/authentication.md`, `services/service-core/.../security/` (`TokenServiceCache` = revalidation/cache, `ActivationTokenService` = FE→BE activation).** | Interim BEFORE (8); first-class token POST-MERGE |
