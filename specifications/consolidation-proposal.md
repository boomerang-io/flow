...
| I6 | Agent transport abstraction | ✅ **RULED: named `AgentProtocol` contract** — one interface + dedicated wire DTOs in the agent module; HTTP v1 stays byte-identical; v2 adds epoch/lease/auth at one seam; unblocks removing `phase` from the public API. |
