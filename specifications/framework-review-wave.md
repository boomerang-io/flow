...
| A7 | No Bean Validation; inline regex checks diverged | `TaskService.java:90`, `WorkflowTemplateService.java:45`, `WorkflowService.java:1572-1600`, `WorkspaceService.java:546-553` | Ruled 2026-09-01: **apply** (shared `ResourceName.REGEX` + `@Valid` edge + `handleMethodArgumentNotValid` → `RestErrorResponse`) | 🟡 IN PROGRESS |
...
| A10 | Hand-built `PageRequest` ×9; wrong `totalElements` | `PageableExecutionUtils.getPage(list, p, () -> list.size())` ×8 | Ruled 2026-09-01: **apply step 1** (9 count fixes + `TokenService:507` `ActionEntity` typo, no wire change); step 2 `Pageable` adoption = issue | 🟡 IN PROGRESS · 🐙 boomerang-io/flow#328 (step 2) |
