...
| Admission is quota-bounded: runs wait `pending` until capacity | Dispatcher fan-out is unbounded: `QueueService.processTaskRun` is `@Async`, one virtual thread per claimed Task (`service-dispatcher/src/main/java/io/boomerang/dispatcher/QueueService.java:48,71`; `config/ThreadConfig.java:33-40`); the only bound is `PAGE_SIZE=20` per 5 s poll (`service-core/src/main/java/io/boomerang/dispatcher/DispatcherService.java:36`) | One `dispatcher.tasks.maxInFlight` ceiling that skips the poll when full; unclaimed work stays in Mongo for other dispatchers. Not the per-type caps that wait for load evidence. |
...
| Retention | `EXECUTIONS_DATA_PRUNE=true`, `MAX_AGE=336` h; annotated executions are never pruned | No age-based run retention for live workflows — only a deleted workflow's documents are pruned, once its runs reach the terminal phase (`WorkflowWatcher.java:267`); time-to-live only on outbox/inbox/locks (`_0018__EventAndLockIndexes.java:53, 71, 96`) |
