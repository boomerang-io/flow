...
| C2 | **Parameter delivery** (rewritten 2026-08-21, maintainer decision) | TWO channels, cleanly split by concern: (a) `$(params.x)` interpolation into script/args/command — moving **engine-side** (`ParameterManager`), no longer the agent's job; (b) env — `PARAMS` (the whole `{name: value}` map as one JSON object, original names and types — the lossless channel `@boomerang-io/task-core` reads, since `PARAM_<NAME>` cannot round-trip names like `privateKey`) plus one `PARAM_<NAME>` per param (name upper-cased, non-`[A-Za-z0-9_]` → `_`; non-string values JSON-encoded; null → `""`); both executors also export `RESULTS_PATH` (Tekton: directory `/tekton/results`; Jobs: file `/dev/termination-log`) plus explicit `TaskEnvVar`s, which win on a name collision with a generated `PARAM_` var (`KubeHelperService.createTaskEnvVars`, both executors). Two params whose sanitised names collide fail the Task (`BoomerangException`, `PARAM_NAME_COLLISION`) rather than silently overwriting each other. The **`/params` projected-ConfigMap file channel is removed** — see the side-objects table above | Runtime obligation is now just (b): export one `PARAM_<NAME>` env var per param, fail loud on a sanitised-name collision, and let explicit task env vars win. `$(params.x)` interpolation is the engine's responsibility, resolved before the spec reaches the runtime — not an any-runtime obligation |
