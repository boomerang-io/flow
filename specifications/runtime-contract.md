...
|---|---|---|
| ~~ConfigMap per TaskRun (param file delivery)~~ **REMOVED (2026-08-21)** | The `/params` projected-ConfigMap channel is gone — params are env-only (`PARAM_<NAME>`, see C2). The only ConfigMap either executor still creates is the Jobs executor's **script ConfigMap** (`{product}-cfg-script-`, task labels, mounted at `/scripts/script`, executable), owned end-to-end by `KubeJobsExecutor` (created in `create`, deleted in `watch`'s `finally`, `cancel`, and `delete` — label-selector delete, idempotent) | `KubeJobsExecutor.java` (`create`/`addScriptOrCommand`, `watch`, `cancel`, `delete`) |
...
| **Triggers / EventListeners** | Absent; dispatch is agent-poll (`EngineClient.java:156-210`, 5s poll) plus, since 2026-09-01, a batched lease heartbeat (`LeaseHeartbeat` → `PUT /api/v1/dispatcher/{id}/heartbeat {ids}` every 30 s; engine lease 90 s) |
...
| C2 | **Parameter delivery** (rewritten 2026-08-21, maintainer decision) | TWO channels, cleanly split by concern: (a) `$(params.x)` interpolation into script/command/args/envs — **engine-side, SHIPPED 2026-08-25** (`ParameterManager.resolveSpec`, persisted by the admission Compare-And-Set `TaskRunService.tryAdmit`), identical on every executor; Tekton's own controller-side substitution still runs but is a no-op on already-substituted text; (b) env — one `PARAM_<NAME>` per param (name upper-cased, non-`[A-Za-z0-9_]` → `_`; non-string values JSON-encoded; null → `""`) plus `PARAM_NAMES` (original names, comma-separated, so `@boomerang-io/task-core` maps `PARAM_PRIVATEKEY` back to `privateKey`). **Ruled 2026-08-22** (see `task-contract-research.md`): these two channels are the contract because any image — Flow-aware or not — can consume them; a `PARAMS` JSON env var was built and removed the same day (128 KiB single-string ceiling, no secret story, only the catalogue would read it). Structured/large inputs are workspace storage, referenced by a param. Both executors export `RESULTS_PATH` (Tekton: directory `/tekton/results`; Jobs: file `/dev/termination-log`). **Payload caps SHIPPED 2026-08-25, engine-enforced so the ceiling is one clear message on every executor**: resolved params ≤ `flow.engine.task.params.max-bytes` (16384) checked at admission — oversize is invalidated via `TaskRunService.tryInvalidate` before it is ever claimable; results ≤ `flow.engine.task.results.max-bytes` (4096, the portable termination-message ceiling) checked in `TaskRunService.end` — oversize fails the task and the results are not persisted plus explicit `TaskEnvVar`s, which win on a name collision with a generated `PARAM_` var (`KubeHelperService.createTaskEnvVars`, both executors). Two params whose sanitised names collide fail the Task (`BoomerangException`, `PARAM_NAME_COLLISION`) rather than silently overwriting each other. The **`/params` projected-ConfigMap file channel is removed** — see the side-objects table above | Runtime obligation is now just (b): export one `PARAM_<NAME>` env var per param, fail loud on a sanitised-name collision, and let explicit task env vars win. `$(params.x)` interpolation is the engine's responsibility, resolved before the spec reaches the runtime — not an any-runtime obligation |
| C3 | **Secrets** | **None.** Only registry auth exists (imagePullSecret, deploy-scoped). No per-task secret mounts, no masked env | Contract today = registry credentials only. Env-only param delivery (C2) means a future `sensitive` param type stays on the *same* `PARAM_<NAME>` env var name, sourced via `valueFrom.secretKeyRef` instead of a literal `value` — no new channel, no new naming scheme. Net-new design, G2-gated (data-model change), not built |
...
| C7 | **Exit-status → RunStatus mapping** | Binary: condition `Succeeded=True` → agent reports `RunStatus.succeeded`; ANY failure/exception → `failed` with the condition reason/message as `statusMessage` (`QueueService.java:79-104`). Exit codes are NOT mapped (only string-sniffed for "exited with code 1" message cleanup); `cancelled`/`timedout` originate engine-side, never from the runtime | Runtime returns `{terminal state: SUCCEEDED\|FAILED\|CANCELLED, reason, message, results}`. Protocol v2 (queue-design D1) requires the dispatcher to map runtime knowledge (exit codes, Tekton reasons, HTTP status) to a **`failureClass`** on `TaskRunEndRequest` — the SPI status object must carry the raw evidence (exit code, reason string) to enable this. **Shipped 2026-09-01 as `statusReason`** (closed set on `TaskRunEndRequest`; `TaskExecutionException.statusReason` in the dispatcher maps Job/TaskRun condition reasons and pod container status — `OOMKilled`, `ImagePull`, `DeadlineExceeded`, `JobDeleted`, `JobFailed`, `ResultsTooLarge`, `AdmissionDenied`, `DispatchError`). `failureClass` is derived from it when retry classes are built, never a second wire field. |
