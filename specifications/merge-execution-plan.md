...
| AM-3 | Proposal step 8's protocol-v2 epoch/lease-renewal did **not** ship — worker leases + fencing are deferred post-merge until a real consumer (lease-reap sweep) exists. I6's "dedicated wire DTOs" overruled: the worker receives the plain run models. **CONSUMED 2026-09-01** (`feat-v5-liveness`): the lease-reap sweep (`reapExpiredLeases`) and the batched dispatcher heartbeat (`PUT /api/v1/dispatcher/{id}/heartbeat`, 30 s beat / 90 s lease) shipped; leases are opt-in by behaviour (no heartbeat = no lease = deadline reaping as before). Evidence that triggered it: 5 of 5 durable peers detect a dead worker in seconds-to-a-minute vs Flow's full task timeout (`competitive-analysis.md` §15). |
