 ...
 * the TaskRun carries no image or command at all and the dispatcher supplies both - the
 * Flow-shipped worker image {@code flow.dispatcher.ai.image} and its {@code prompt} command. Any
 * image, command or script that somehow reached the spec of an {@code ai} task is ignored, so a
 * definition cannot redirect the AI worker at another container.
 *
