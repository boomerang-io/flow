...
class TaskLockTest extends AbstractEngineIntegrationTest {
  ...
  void oneTaskHoldsAContendedLock() {
    assertNotNull(taskLockRepository.tryAcquire("ws:deploy", "task-a", "wf1", future()));
    assertNull(
        taskLockRepository.tryAcquire("ws:deploy", "task-b", "wf2", future()),
        "a live lock held by another task is never stolen");

    // A non-holder cannot release it.
    taskLockRepository.release("ws:deploy", "task-b");
    assertNull(taskLockRepository.tryAcquire("ws:deploy", "task-b", "wf2", future()));

    // The holder releases; the key is now free.
    taskLockRepository.release("ws:deploy", "task-a");
    assertNotNull(taskLockRepository.tryAcquire("ws:deploy", "task-b", "wf2", future()));
  }

  @Test
  void expiredLeaseIsTakenOver() {
    Date past = new Date(System.currentTimeMillis() - 1000);
    assertNotNull(taskLockRepository.tryAcquire("ws:expired", "task-a", "wf1", past));
    assertNotNull(
        taskLockRepository.tryAcquire("ws:expired", "task-b", "wf2", future()),
        "an expired lease is reclaimed by the next acquirer");
  }

  @Test
  void sameKeyInDifferentWorkspacesDoesNotCollide() {
    assertNotNull(taskLockRepository.tryAcquire("ws1:deploy", "task-a", "wf1", future()));
    assertNotNull(
        taskLockRepository.tryAcquire("ws2:deploy", "task-b", "wf2", future()),
