...
public class OutboxDispatcher {
  ...
  public void drain() {
    for (EventOutboxEntity row : eventOutboxRepository.findDeliverable(new Date(), PAGE_SIZE)) {
      try {
        deliver(row);
        if (eventOutboxRepository.tryMarkSent(row.getId(), new Date()) != null) {
          LOGGER.debug("[{}] Outbox row delivered ({} {}).", row.getId(), row.getRefType(), row.getRef());
        }
      } catch (Exception ex) {
        int attempts = row.getAttempts() + 1;
        if (attempts >= MAX_ATTEMPTS) {
          if (eventOutboxRepository.tryMarkDead(row.getId()) != null) {
            LOGGER.error(
                "[{}] Outbox row dead after {} delivery attempts ({} {}): {}",
                row.getId(), attempts, row.getRefType(), row.getRef(), ex.getMessage());
          }
        } else {
          eventOutboxRepository.tryRequeueDelivery(row.getId(), nextRetryAt(attempts), attempts);
          LOGGER.warn(
